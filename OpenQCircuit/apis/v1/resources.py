...
from OpenQCircuit.apis.v1.auth import auth_required, generate_token
from OpenQCircuit.apis.v1.errors import api_abort, ValidationError
from OpenQCircuit.apis.v1.schemas import user_schema, item_schema, items_schema
...
def get_item_name():
    ...
    if name is None or str(name).strip() == '':
        raise ValidationError('The item name was empty or invalid.')
    return name
...
class ItemsAPI(MethodView):
    ...
    def get(self):
        ...
        per_page = current_app.config['APP_ITEM_PER_PAGE']
        pagination = Circuit.query.with_parent(g.current_user).paginate(page, per_page)
    ...
    def post(self):
        """Create new item."""
        item = Circuit(name=get_item_name(), author=g.current_user)
        db.session.add(item)
        db.session.commit()
...
class ActiveItemsAPI(MethodView):
    ...
    def get(self):
        ...
        page = request.args.get('page', 1, type=int)
        pagination = Circuit.query.with_parent(g.current_user).filter_by(done=False).paginate(
            page, per_page=current_app.config['APP_ITEM_PER_PAGE'])
        items = pagination.items
        current = url_for('.items', page=page, _external=True)
        prev = None
        if pagination.has_prev:
...
class CompletedItemsAPI(MethodView):
    ...
    def get(self):
        ...
        page = request.args.get('page', 1, type=int)
        pagination = Circuit.query.with_parent(g.current_user).filter_by(done=True).paginate(
            page, per_page=current_app.config['APP_ITEM_PER_PAGE'])
        items = pagination.items
        current = url_for('.items', page=page, _external=True)
        prev = None
    ...
    def delete(self):
        ...
        Circuit.query.with_parent(g.current_user).filter_by(done=True).delete()
