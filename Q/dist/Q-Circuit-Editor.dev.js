...
Q.Circuit.Editor = function (circuit, targetEl) {
  ...
  toolbarEl.classList.add('Q-circuit-toolbar'); // Create an Add Qubit button
  // It adds another qubit to the circuit

  var addQubitBtn = createDiv();
  toolbarEl.appendChild(addQubitBtn);
  addQubitBtn.classList.add('Q-circuit-button');
  addQubitBtn.setAttribute('title', 'Add Qubit');
  addQubitBtn.innerHTML = '+'; //  Create a toggle switch for locking the circuit.
  ...
  for (var _i = 0; _i < circuit.timewidth; _i++) {
    ...
    columnEl.style.gridColumnStart = _i + 3;
    columnEl.setAttribute('moment-index', _i + 1);
  ...
  for (var _i2 = 0; _i2 < circuit.bandwidth; _i2++) {
    var registerIndex = _i2 + 1,
...
Q.Circuit.Editor.onPointerRelease = function (event) {
  ...
  draggedOperations.forEach(function (childEl, i) {
    ...
    if (registerIndicesString) {
      ...
      var _registerIndices = registerIndicesString.split(',').map(function (str) {
        return +str;
      }),
      ...
      component1 = Q.Circuit.Editor.dragEl.querySelector("[moment-index=\"".concat(childEl.origin.momentIndex, "\"]") + "[register-index=\"".concat(_registerIndices[1], "\"]")),
      ...
      if (_registerIndices.length === foundComponents.length) {
        //circuit.set$( 
        ...
        _registerIndices.map(function (registerIndex) {
      ...
      //  that might be ok!
          ...
          var registerMap = _registerIndices.reduce(function (registerMap, registerIndex, r) {
